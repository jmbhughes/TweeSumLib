...

{
    ...
    # This (along the sub definition) is the only change compared to 
    # default-reranker.pl
    identify_groups();
...
sub identify_groups {
    ...
    foreach my $sentref (@all_sents) {
        $total_score += $$sentref{'Score'};
    }

    my $cutoff_score = $total_score / scalar(@all_sents);

    #print STDERR "Average Sentence Score: $cutoff_score\n";

    #
    # Multiply the score of any sentence with a high score by 1.2
    #
    ...
    foreach my $sentref (@all_sents) {
        my $this_score = $$sentref{'Score'};

        if ($last_score > $cutoff_score && $this_score > $cutoff_score) {
            $$sentref{'Score'} *= $MULTIPLIER;
        }

        # save the last score (from before we _possibly_ modified it).
