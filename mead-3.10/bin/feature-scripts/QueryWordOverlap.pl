...
#
# usage:
#   echo cluster_filename | ./QueryWordOverlap.pl \
...
sub sentence {
	...
	$$feature_vector{'QueryTitleWordOverlap'} = 
	    unigram_overlap($query_title, $sentence_text);
    }

    if (($option eq "all" or $option eq "narrative") and $query_narrative) {
	$$feature_vector{'QueryNarrativeWordOverlap'} = 
	    unigram_overlap($query_narrative, $sentence_text);
    }

    if (($option eq "all" or $option eq "description") and $query_description) {
	$$feature_vector{'QueryDescriptionWordOverlap'} = 
	    unigram_overlap($query_description, $sentence_text);
    }
